...
function seachList(pcurrenpPge){
    ...
    $.ajax({
        url: '/admin/ajax/answered/list',
        type: 'POST',
        dataType: 'json',
        data: 'currenpPge='+pcurrenpPge+'&questionid='+questionid+'&_token='+token,
        beforeSend: function () {
            loadingstart();
        },
    })
        .done(function(r) {
            ...
            if(r.status == 1){
                $.each(r.data,function (name,vale) {
                    ...
                    html += '<td><span style="display:inline-block;width: 300px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">'+vale.detail+' </span></td>';

                    html += '<td>'+vale.display_name+' </td>';
                    html += '<td>'+vale.created_at+' </td>';
                    html += '<td>';
                    html += '<button  onclick = "edit('+vale.id+',\''+vale.detail+'\')" class="btn btn-xs btn-success">编辑</button> ';
                    html += '<button  onclick = "del('+vale.id+')" class="btn btn-xs btn-success">删除</button> ';

                    html += '</td>';
                    html +='</tr>';
                })
                $('#answeredTbody').html(html);
                tatol = r.total;
...
function del(id) {
    pub_alert_confirm('/admin/ajax/answered/del',{id:id},'删除成功!');
}
