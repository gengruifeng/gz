...
<section id="section">
    ...
    <form class="release" action="">
        <div>
            <input type="text" placeholder="标题 : 一句话描述问题，用问号结尾" id="subject" value="{{$askdetail->subject}}" onkeyup="checkMaxInput(this,'inputnumber',50)" onkeydown="checkMaxInput(this,'inputnumber',50)"/>
            <p>还可以输入
                <a href="javascript:void(0)" id="inputnumber"></a>字</p>
        </div>
        <div>
            <div>
                {{ csrf_field() }}
                <input type="hidden" value="{{$askdetail->id}}" class="_token" id="askid">
                <textarea id="editor" placeholder="请输入...">{{$askdetail->detail}}</textarea>
            </div>
        </div>
        <div>
            <div>
                <div id="addTag" style="display: none;"></div>
                <div class="clearfix demo">
                    <input type="text" id="search_tag" value="{{$askdetail->tags}}">
                </div>
                <div>
                    <button type="button" id="askedit">发布</button>
                </div>
            </div>
