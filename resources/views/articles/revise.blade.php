...
<section id="section">
    <!--发布文章开始-->
    <form class="release" id="article-form" action="/ajax/articles/{{ $id }}">
        <div>
            <input type="text" placeholder="请输入您的文章标题" id="subject" value="{{ $subject }}" onkeyup="checkMaxInput(this,'inputnumber',50)" onkeydown="checkMaxInput(this,'inputnumber',50)"/>
            <p>还可以输入
                <a href="javascript:void(0)" id="inputnumber">50</a>字</p>
        </div>
        <div>
            <div>
                <textarea id="editor" placeholder="请输入...">{{ $detail }}</textarea>
            </div>
            <div><p></p></div>
        </div>
        <div>
            <div>
                ...
                <div class="demo">
                    <input type="text" id="search_tag" value="{{ $tags }}">
                </div>
                <div>
                    <button type="button" id="compose">发布</button>
                </div>
            </div>
...
<script src="{{ asset('js/articleadd.js') }}"></script>
